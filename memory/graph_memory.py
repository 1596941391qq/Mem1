...
class MemoryGraph:
    ...
    def add(self, data, filters):
        ...
        if self.config.graph_store.custom_prompt:
            messages = [
                {"role": "system",
                 "content": EXTRACT_ENTITIES_PROMPT.replace("USER_ID", self.user_id).replace("CUSTOM_PROMPT",
        ...
        if extracted_entities['tool_calls']:
            ...
            match = re.search(r'(\[.*?])', extracted_entities['content'], re.DOTALL)
            try:
                extracted_entities = match.group(0)
        ...
        for item in memory_updates['tool_calls']:
            if item['name'] == "add_graph_memory":
                ...
                self._update_relationship(item['arguments']['source'], item['arguments']['destination'],
                                          item['arguments']['relationship'], filters)
            elif item['name'] == "noop":
                continue
